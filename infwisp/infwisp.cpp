...
void threadCalc(int mod,int modLT,int startNum,int endNum,int isSaveOrNo,int addLT, int pinLT,int decLT,int helLT,int arcLT,int splLT) {
    int out = 1;
    int YouNeedNum;
    int Count = 0;
    int test;
...
int main() {//addLT蓝表,pinLT乒乓,decLT红表，helLT螺旋魔弹,arcLT相位弧度,splLT连锁法术
    ...
    while (true)
    {
        ...
        if (splLT != 0 && decLT != 0) {
            ...
            if (cfg.getParameter("threadNum") > 1) {
                T = vector<thread>(cfg.getParameter("threadNum") - 1); //初始化线程数量，主线程也算一个，所以-1
                TNum = vector<int>(cfg.getParameter("threadNum"));
            }
            int PreNumI = -decLT * 42 - splLT * 30 + endNum;//预计算，减少性能消耗
            for (int add = 0; add <= addLT; add++)
            {
